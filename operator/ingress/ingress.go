...
type ingressAddedEvent struct {
	ingress *slimnetworkingv1.Ingress
}
type ingressUpdatedEvent struct {
	oldIngress *slimnetworkingv1.Ingress
	newIngress *slimnetworkingv1.Ingress
}
...
type serviceAddedEvent struct {
	service *slimcorev1.Service
}
type serviceUpdatedEvent struct {
	oldService *slimcorev1.Service
	newService *slimcorev1.Service
}
...
func (ic ingressController) handleAddService(obj interface{}) {
	service, ok := obj.(*slimcorev1.Service)
	if !ok {
		return
	}
...
func (ic ingressController) handleUpdateService(oldObj, newObj interface{}) {
	oldService, ok := oldObj.(*slimcorev1.Service)
	if !ok {
		return
	}
	newService, ok := newObj.(*slimcorev1.Service)
	if !ok {
		return
	}
...
func (ic ingressController) handleDeleteService(obj interface{}) {
	...
	case *slimcorev1.Service:
		ic.queue.Add(serviceDeletedEvent{service: concreteObj})
	case cache.DeletedFinalStateUnknown:
		service, ok := concreteObj.Obj.(*slimcorev1.Service)
		if ok {
...
func toV1TypeMeta(in slimmetav1.TypeMeta) metav1.TypeMeta {
...
func toV1ObjectMeta(in slimmetav1.ObjectMeta) metav1.ObjectMeta {
...
func toV1IngressStatus(in slimcorev1.LoadBalancerStatus) v12.IngressStatus {
...
func getIngressForStatusUpdate(slimIngress *slimnetworkingv1.Ingress, lb slimcorev1.LoadBalancerStatus) *v12.Ingress {
...
func (ic *ingressController) syncLoadBalancerIPs(service *slimcorev1.Service) error {
...
func (ic *ingressController) getIngressFromCache(key string) (*slimnetworkingv1.Ingress, error) {
	...
	ingress, ok := objFromCache.(*slimnetworkingv1.Ingress)
	if !ok {
...
func getServiceNameForIngress(ingress *slimnetworkingv1.Ingress) string {
	return ciliumIngressPrefix + ingress.Name
}

func getIngressKeyForService(service *slimcorev1.Service) string {
	ingressName := strings.TrimPrefix(service.Name, ciliumIngressPrefix)
	return fmt.Sprintf("%s/%s", service.Namespace, ingressName)
}

func getServiceForIngress(ingress *slimnetworkingv1.Ingress) *v1.Service {
...
func getEndpointsForIngress(ingress *slimnetworkingv1.Ingress) *v1.Endpoints {
	return &v1.Endpoints{
		ObjectMeta: metav1.ObjectMeta{
			Name:      getServiceNameForIngress(ingress),
			Namespace: ingress.Namespace,
...
func (ic *ingressController) createLoadBalancer(ingress *slimnetworkingv1.Ingress) error {
	...
	if exists {
		_, ok := objFromCache.(*slimcorev1.Service)
		if !ok {
		...
		// controller.
		ic.logger.WithField("service", svcKey).Info("Service already exists. Continuing...")
		return nil
	}
	_, err = k8s.Client().CoreV1().Services(ingress.Namespace).Create(context.Background(), svc, metav1.CreateOptions{})
	if err != nil {
		ic.logger.WithError(err).WithField("ingress", ingress.Name).Error("Failed to create a service for ingress")
	} else {
		ic.logger.WithError(err).WithField("service", svcKey).Info("Created Service")
	}
	return err
}

func (ic *ingressController) createEndpoints(ingress *slimnetworkingv1.Ingress) error {
	...
	if exists {
		_, ok := objFromCache.(*slimcorev1.Endpoints)
		if !ok {
			return fmt.Errorf("got invalid object from cache")
		}
		// Endpoints already exists in the cache. For now assume that it was created by the ingress
		// controller.
		ic.logger.WithField("endpoints", key).Info("Endpoints already exists. Continuing...")
		return nil
	}
	_, err = k8s.Client().CoreV1().Endpoints(ingress.Namespace).Create(context.Background(), endpoints, metav1.CreateOptions{})
	if err != nil {
		ic.logger.WithError(err).WithField("ingress", ingress.Name).Error("Failed to create endpoints for ingress")
	} else {
		ic.logger.WithError(err).WithField("endpoints", key).Info("Created Endpoints")
	}
	return err
}

func (ic *ingressController) createEnvoyConfig(ingress *slimnetworkingv1.Ingress) error {
	...
	if exists {
		...
		// controller.
		ic.logger.WithField("cilium-envoy-config", key).Info("CiliumEnvoyConfig already exists. Continuing...")
		return nil
	}
	_, err = k8s.CiliumClient().CiliumV2alpha1().CiliumEnvoyConfigs().Create(context.Background(), envoyConfig, metav1.CreateOptions{})
	if err != nil {
		ic.logger.WithError(err).WithField("ingress", ingress.Name).Error("Failed to create CiliumEnvoyConfig for ingress")
	}
	return err
}

func (ic *ingressController) deleteEndpoints(ingress *slimnetworkingv1.Ingress) error {
	endpoints := getEndpointsForIngress(ingress)
	key, err := cache.MetaNamespaceKeyFunc(endpoints)
	...
	if !exists {
		ic.logger.WithField("endpoints", key).Info("Endpoints already deleted. Continuing...")
		return nil
	}
	err = k8s.Client().CoreV1().Endpoints(ingress.Namespace).Delete(context.Background(), endpoints.Name, metav1.DeleteOptions{})
	if err != nil {
		ic.logger.WithError(err).WithField("ingress", ingress.Name).Error("Failed to delete endpoints for ingress")
	}
	return err
}

func (ic *ingressController) deleteLoadBalancer(ingress *slimnetworkingv1.Ingress) error {
...
func (ic *ingressController) handleAddIngress(obj interface{}) {
	...
	ingress, ok := obj.(*slimnetworkingv1.Ingress)
	if !ok {
...
func (ic *ingressController) handleUpdateIngress(oldObj, newObj interface{}) {
	oldIngress, ok := oldObj.(*slimnetworkingv1.Ingress)
	if !ok {
		return
	}
	newIngress, ok := newObj.(*slimnetworkingv1.Ingress)
	if !ok {
		return
	}
...
func (ic *ingressController) handleDeleteIngress(obj interface{}) {
	switch concreteObj := obj.(type) {
	case *slimnetworkingv1.Ingress:
		ic.queue.Add(ingressDeletedEvent{ingress: concreteObj})
	case cache.DeletedFinalStateUnknown:
		ingress, ok := concreteObj.Obj.(*slimnetworkingv1.Ingress)
		if ok {
...
func (ic *ingressController) deleteCiliumEnvoyConfig(ingress *slimnetworkingv1.Ingress) error {
	...
	if !exists {
		ic.logger.WithField("cilium-envoy-config", ingress.Name).Info("CiliumEnvoyConfig already deleted. Continuing...")
		return nil
	}
	err = k8s.CiliumClient().CiliumV2alpha1().CiliumEnvoyConfigs().Delete(context.Background(), ingress.Name, v1meta.DeleteOptions{})
	if err != nil {
		ic.logger.WithError(err).WithField("cilium-envoy-config", ingress.Name).Error("Failed to delete CiliumEnvoyConfig for ingress")
	} else {
		ic.logger.WithField("cilium-envoy-config", ingress.Name).Info("Deleted CiliumEnvoyConfig")
	}
	return err
}
